...
class CBC (object):
    ...
    def CNN_layer_op(self):
        ...
        with tf.variable_scope('convolution'):
            ...
            ss = tf.reduce_max(self.sequence_lengths)

            conv = tf.nn.relu(tf.nn.conv2d(cnn_input, W, strides=[1, 1, 1, 1], padding='VALID') + b)
            char_represent = tf.nn.max_pool(conv, ksize=[1, self.words_lengths-2, 1, 1],strides=[1,1,1,1], padding='VALID')
    ...
    def biLSTM_layer_op(self):
        ...
        with tf.variable_scope("bi-lstm"):
            ...
            (output_fw_seq, output_bw_seq), _ = tf.nn.bidirectional_dynamic_rnn(
                ...
                dtype=tf.float32)
            self.lstm_outputs = tf.concat([output_fw_seq, output_bw_seq], axis=-1)
    ...
    def trainstep_op(self):
        ...
        with tf.variable_scope("train_step"):
            ...
            if self.optimizer == 'Adam':
                optim = tf.train.AdamOptimizer(learning_rate=self.lr_pl, beta1 = 0.9)
    ...
    def run_one_epoch(self,sess, train, dev, tag2label, epoch, saver):
        ...
        for step, (seqs, words, labels) in enumerate(batches):
            ...
            concat_operation,lstm_outputs,_, _, loss_train, summary, step_num_ = sess.run([self.concat_operation,
                                                                                           self.lstm_outputs,self.train_op,
